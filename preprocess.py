...
def load_data(filename, entity2id, relation2id, is_unweigted=False, directed=True):
    ...
    for line in lines:
        ...
        if relation2id[relation] not in relation2id_num:
            relation2id_num[relation2id[relation]] = 0
        else:
            relation2id_num[relation2id[relation]] += 1
        ...
        if not directed:
            ...
            if is_unweigted:
                ...
                rel_num.append(relation2id_num[relation2id[relation]])
...
def build_data(path='./data/WN18RR/', is_unweigted=False, directed=True):
    ...
    train_triples, train_adjacency_mat, unique_entities_train, relation2id_num = load_data(os.path.join(
        path, 'train.txt'), entity2id, relation2id, is_unweigted, directed)
    validation_triples, valid_adjacency_mat, unique_entities_validation, _ = load_data(
        os.path.join(path, 'valid.txt'), entity2id, relation2id, is_unweigted, directed)
    test_triples, test_adjacency_mat, unique_entities_test, _ = load_data(os.path.join(
    ...
    return (train_triples, train_adjacency_mat), (validation_triples, valid_adjacency_mat), (test_triples, test_adjacency_mat), \
        entity2id, relation2id, relation2id_num, headTailSelector, unique_entities_train
