...
import queue
import random
from HK_random import HKrandom
import pdb


class Corpus:
    def __init__(self, args, train_data, validation_data, test_data, entity2id,
                 relation2id, relation2id_num, headTailSelector, batch_size, valid_to_invalid_samples_ratio, unique_entities_train, get_2hop=False, get_reverse2hop=False):
        ...
        rel_num = torch.LongTensor(train_data[1][3])
        self.train_adj_matrix = (adj_indices, adj_values, rel_num)
        ...
        self.relation2id_num = relation2id_num

        if(get_2hop):
            self.graph = self.get_graph()
            self.node_neighbors_2hop = self.get_further_neighbors(self.graph)

        if(get_reverse2hop):
            self.reverse_graph = self.reverse_get_graph()
            self.reverse_node_neighbors_2hop = self.get_further_neighbors(self.reverse_graph)
        # pdb.set_trace()
    ...
    def reverse_get_graph(self):
        # pdb.set_trace()
        # create graph for the original directed. consider the outgoing edge
        graph = {}
    ...
    def dfs(self, graph, source, visit):
        path=[]
        visit[source] = 1
        q = queue.Queue()
        q.put(source)
        ...
        while(not q.empty()):
            ...
            if top in graph.keys():
                graph_top = list(graph[top].keys())
                if len(graph_top)>0:
                    random.shuffle(graph_top)
                    target = graph_top[0]
                    if(target in visit.keys()):
                        continue
                    else:
                        q.put(target)
                        path.append([top, graph[top][target], target])
                        visit[target] = 1
        # pdb.set_trace()

        return path, visit

    def get_further_neighbors(self, graph, nbd_size=2):
        neighbors = {}
    ...
    def get_control_npathHK_all(self, args, nodes, edges):
        # pdb.set_trace()
        ...
        # pdb.set_trace()
        print(len(newallpath))
        print("average length:"+ str(alllen_path/len(newallpath)))
        ...
        return edge_path,all_node_list,all_node_list_mask,all_rel_list,all_rel_list_mask

    def get_random_npath(self, args):
        # pdb.set_trace()
        # get random path starting from random node, util include all node.
        # nodes=[0,1,2,3,4,5]
        # edges=[(0,101,1),(1,102,2),(1,103,3),(2,104,4),(3,105,4),(2,106,5)]
        ...
        for i in range(args.n_path):
            ...
            for source in nodes_list:
                # st_time = time.time()
                if (source not in visit.keys()):
                    path, visit = self.dfs(graph, source, visit)
                    if len(path)>1:
                        allpath.append(path)
    ...
    def get_validation_pred(self, args, model, unique_entities):
        ...
        for iters in range(1):
            ...
            for i in range(batch_indices.shape[0]):
                ...
                if ('WN' in args.data) or ("NELL-995" in args.data):
                    num_triples_each_shot = int(
                        math.ceil(new_x_batch_head.shape[0] / 6))
                    ...
                    scores_head = torch.cat(
                        [scores1_head, scores2_head, scores3_head, scores4_head, scores5_head, scores6_head], dim=0)
                ...
                if ('WN' in args.data) or ("NELL-995" in args.data):
                    num_triples_each_shot = int(
                        math.ceil(new_x_batch_tail.shape[0] / 6))
                    ...
                    scores_tail = torch.cat(
                        [scores1_tail, scores2_tail, scores3_tail, scores4_tail, scores5_tail, scores6_tail], dim=0)
        ...
        return cumulative_hits_100, cumulative_hits_ten, cumulative_hits_three, \
