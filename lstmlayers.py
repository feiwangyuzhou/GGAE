...
class StackedBRNN(nn.Module):
    ...
    def forward(self, x, x_mask=None, return_list=False, additional_x=None, previous_hiddens=None):
        ...
        # pdb.set_trace()
        x_mask=x_mask.unsqueeze(2).expand(x.size(0),x.size(1),x.size(2)).contiguous()
        x_mask=x_mask.float()
        x=x*x_mask
        x = x.transpose(0, 1)
...
class MTLSTM(nn.Module):
    ...
    def forward(self, x_idx, x_mask):
        ...
        """
        # how to removed emb tune-partial?
        # emb = self.embedding
        emb = self.embedding if self.training else self.eval_embed
        x_hiddens = emb(x_idx)
        q_len = x_mask.size()[-1]  # the question length for this batch 
        ...
        output1 = unpack(output1, batch_first=True, total_length=q_len)[0]
        output2 = unpack(output2, batch_first=True, total_length=q_len)[0]
